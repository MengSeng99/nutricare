...
class _AdminSpecialistsScreenState extends State<AdminSpecialistsScreen>
  ...
  int dietitianCount = 0;

  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this);
    _fetchSpecialistCounts();
  }
  ...
  Future<void> _fetchSpecialistCounts() async {
    // Count active nutritionists
    final nutritionistSnapshot = await FirebaseFirestore.instance
        .collection('specialists')
        .where('specialization', isEqualTo: 'Nutritionist')
        .get(); // Get all nutritionists regardless of status

    // Count active dietitians
    final dietitianSnapshot = await FirebaseFirestore.instance
        .collection('specialists')
        .where('specialization', isEqualTo: 'Dietitian')
        .get(); // Get all dietitians regardless of status

    // Calculate active specialists by checking the existence of the status field
    setState(() {
      nutritionistCount = nutritionistSnapshot.docs.where((doc) =>
          !doc.data().containsKey('status')).length; // Check if status field is absent
      dietitianCount = dietitianSnapshot.docs.where((doc) =>
          !doc.data().containsKey('status')).length; // Check if status field is absent
    });
  }
  @override
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            Tab(text: 'Nutritionists ($nutritionistCount)'),
            Tab(text: 'Dietitians ($dietitianCount)'),
          ],
        ),
        actions: [
          IconButton(
...
class SpecialistsListView extends StatefulWidget {
  ...
  @override
  _SpecialistsListViewState createState() => _SpecialistsListViewState();
}

class _SpecialistsListViewState extends State<SpecialistsListView> {
  // Manage visibility of inactive specialists
  bool showInactiveSpecialists = false;

  int inactiveSpecialistCount = 0;
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final activeSpecialists = specialists.where((specialist) {
          ...
          return name.toLowerCase().startsWith(widget.searchKeyword) &&
              status != 'inactive';
        }).toList();
        ...
        inactiveSpecialistCount = inactiveSpecialists.length;

        if (activeSpecialists.isEmpty) {
          return Center(
              child: Text(
                  "No active specialists found matching '${widget.searchKeyword}'"));
        }
        ...
        return Column(
          children: [
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final email = specialistData['email'] ?? 'No Email';
                  final specialistId = activeSpecialists[index].id;
            ...
            // Show/hide button for inactive specialists
            if (inactiveSpecialists.isNotEmpty) ...[
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                  Text(
                      'Inactive Specialists  ($inactiveSpecialistCount)',
                      style:
                          TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
                    ),
                    IconButton(
                      icon: Icon(
                        showInactiveSpecialists
                            ? Icons.expand_less
                            : Icons.expand_more,
                        color: Color.fromARGB(255, 90, 113, 243),
                      ),
                      onPressed: () {
                        setState(() {
                          showInactiveSpecialists =
                              !showInactiveSpecialists; // Toggle visibility
                        });
                      },
                    ),
                  ],
              ...
              if (showInactiveSpecialists) // Only show this list if the flag is true
                Expanded(
                  child: ListView.builder(
                    itemCount: inactiveSpecialists.length,
                    itemBuilder: (context, index) {
                      final specialistData = inactiveSpecialists[index].data()
                          as Map<String, dynamic>;
                      final profilePictureUrl =
                          specialistData['profile_picture_url'] ?? '';
                      final email = specialistData['email'] ?? 'No Email';
                      final specialistId = inactiveSpecialists[index].id;
  ...
  Widget _buildInactiveSpecialistCard(
    ...
    final String deactivationDate = deactivationTimeStamp != null
        ? "${deactivationTimeStamp.toDate()}"
            .split(' ')[0] // Formatting to only get the date part
        : 'Unknown';
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
                'Deactivated on: $deactivationDate', // Displaying deactivation date
